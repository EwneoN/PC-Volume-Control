	...
	public class VolumeControl
	{
		...
		private static bool GetMixer(int i, int type, int ctrlType, out VolumeStructs.Mixer mixerControl, out int currVolume)
		{
			...
			if (VolumeConstants.MMSYSERR_NOERROR == details)
			{
				...
				detailsUnsigned =
					(VolumeStructs.UnsignedMixerDetails)
						Marshal.PtrToStructure(mcDetails.paDetails, typeof (VolumeStructs.UnsignedMixerDetails));
		...
		public static void SetVolume(int volumeLevel)
		{
			try
			{
				...
				GetMixer(mixerControl, VolumeConstants.MIXERLINE_COMPONENTTYPE_DST_SPEAKERS, controlType, out volumeControl,
					out currVolume);
